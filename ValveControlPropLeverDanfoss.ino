...
#include "EEPROM.h"
//for danfoss valve per chrisc
#include <SPI.h>
#include <Adafruit_MCP4725.h>
#include <Wire.h>

Adafruit_MCP4725 dac;
float dfValue = .500; //set danfoss valve at middle
bool danfoss = true; //set true for danfoss
int pwm = 2048; //send to MCP4725
int dfout = 50;
...
#define BOFFDW_PIN 6
bool bladeOffsetButton = false; // true if this fonctionality is used
...
void loop()
{
  ...
  if (currentTime - lastTime >= LOOP_TIME)
  {
    ...
    if (watchdogTimer < 29) {
      ...
      //LeverSideValue = analogRead(LEVER_SIDE); not read
    ...
    if (Serial.available() > 5 && isDataFound)
    {
      ...
      bladeOffsetOut = 100;
      Serial.print(",");
      Serial.print(dfValue*5); //just for info, not used
      Serial.print(",");
      Serial.print(pwmValueCalc); //just for info, not used
      Serial.print(",");
      Serial.print(pwm); //just for info, not used //(LeverPushValue);
      Serial.print(",");
...
void SetPWM() {
  ...
  */
  ...
  if (true)                      // always true for danfoss (workSwitch == 0 && autoEnable == 1)// Auto mode
  {
    ...
    pwmValueCalc= (float)(pwmValue*1.00);
  //  end of automode remove manual mode for danfoss testing per chrisc
  /*else // if manual mode
    {
    ...
    dfValue =(256 + pwmValueCalc) / 512.000;
    pwm = dfValue * 4096;
    dfout = dfValue*100;
